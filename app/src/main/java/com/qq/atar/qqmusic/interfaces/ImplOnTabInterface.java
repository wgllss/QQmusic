...
public class ImplOnTabInterface<V extends ViewGroup, A extends CommonActivity> implements OnTabInterface<V, A>, OnPageChangeListener {
    ...
    public void setViewPagerAdapter() {
        if (mFragmentActivity != null && mViewPager != null && getFragmentList() != null && mFragmentActivity.getSupportFragmentManager() != null) {
            ...
            if (frameMove != null) {
                CommonTraslate.TranslateAnimation(frameMove, startLeft, 0 + lineMarginLeft, 0, 0, 0, null);
            }
        }
    }
    ...
    public void onPageSelected(int arg0) {
        if (frameMove != null) {
            CommonTraslate.TranslateAnimation(frameMove, startLeft, arg0 * frameMove.getWidth() + lineMarginLeft, 0, 0, 400, null);
        }
        if (mLinearLayout != null) {
            for (int i = 0; i < tabCount; i++) {
                if (arg0 == i) {
                    SkinUtils.setTextColor(mFragmentActivity, R.string.array_color_common_select_tab_text_color, mFragmentActivity.getCurrentSkinType(), ((TextView) mLinearLayout.getChildAt(i)));
                } else {
                    SkinUtils.setTextColor(mFragmentActivity, R.string.array_color_common_tab_text_color, mFragmentActivity.getCurrentSkinType(), ((TextView) mLinearLayout.getChildAt(i)));
                }
            }
        }
        if (frameMove != null) {
            startLeft = arg0 * frameMove.getWidth() + lineMarginLeft;
        }
    ...
    public void setLineMarginLeft(int lineMarginLeft) {
        this.lineMarginLeft = (int) ScreenUtils.getIntToDip(lineMarginLeft);
        if (frameMove != null) {
            V.LayoutParams lp = (V.LayoutParams) frameMove.getLayoutParams();
            lp.width = (DeviceManager.getScreenWidth(mFragmentActivity) - 2 * this.lineMarginLeft) / tabCount;
            frameMove.setLayoutParams(lp);
        }
    }
    ...
    public void setChangeTabSkin(int skinType) {
        SkinUtils.setBackgroundColor(mFragmentActivity, R.string.array_color_common_white_color, skinType, mLinearLayout);
        SkinUtils.setBackgroundColor(mFragmentActivity, R.string.array_color_common_tab_linebg_color, skinType, viewMove);
//        SkinUtils.setBackgroundColor(mFragmentActivity, R.string.array_color_common_content_bg_color, skinType, mLinearLayout);
        if (mLinearLayout != null) {
            for (int i = 0; i < tabCount; i++) {
                SkinUtils.setTextColor(mFragmentActivity, R.string.array_color_common_tab_text_color, skinType, ((TextView) mLinearLayout.getChildAt(i)));
            }
            SkinUtils.setTextColor(mFragmentActivity, R.string.array_color_common_select_tab_text_color, skinType, ((TextView) mLinearLayout.getChildAt(getCurrentItem())));
        }
    }
}
